...
func (d *dataItemWrap) Distribute(nodeID hraftd.NodeID, item hraftd.Identifier) {
	d.NodeID = nodeID
...
func TestDistribute(t *testing.T) {
	d := hraftd.NewDistributor()
	peers := []hraftd.Peer{{ID: "1"}, {ID: "2"}, {ID: "3"}}
	...
	assert.Equal(t, map[string]hraftd.NodeID{"1": "1", "2": "2", "3": "3"}, d.StickyMap)
	...
	assert.Equal(t, map[string]hraftd.NodeID{"1": "1", "4": "1", "2": "2", "3": "3"}, d.StickyMap)
	...
	assert.Equal(t, map[string]hraftd.NodeID{"1": "1", "3": "3", "4": "1", "6": "3", "7": "1", "5": "3"}, d.StickyMap)

	peers = []hraftd.Peer{{ID: "1"}}
	data = []dataItem{{iid: "1"}, {iid: "7"}, {iid: "3"}, {iid: "4"}, {iid: "5"}, {iid: "6"}}
	assert.Equal(t, []dataItemWrap{
		{NodeID: "1", dataItem: dataItem{iid: "1"}},
		{NodeID: "1", dataItem: dataItem{iid: "7"}},
	...
	assert.Equal(t, map[string]hraftd.NodeID{"1": "1", "7": "1", "4": "1", "3": "1", "5": "1", "6": "1"}, d.StickyMap)
